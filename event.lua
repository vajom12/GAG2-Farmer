...
if ENV.GAG2_EVENT_V3_STOP then
    pcall(ENV.GAG2_EVENT_V3_STOP)
end

local running = true
local busy = false
ENV.GAG2_EVENT_V3_STOP = function() running = false end
...
local function notify(text)
    print("[GAG2 V3 EVENT]", text)
    if not CFG.Notify then return end
    pcall(function()
        StarterGui:SetCore("SendNotification", {
            Title = "GAG2 V3 Event Hunter",
            Text = text,
            Duration = 5,
        })
    end)
...
local function reach(pos)
    ...
    local target = pos + Vector3.new(0, CFG.ReachHeight, 0)
    setCollide(false)
    for _ = 1, 80 do
        ...
        for _, obj in ipairs(folder:GetChildren()) do
            if not seen[obj] then
                ...
                if pos and (prompt or marked or kind ~= "Unknown") then
                    if not CFG.RareOnly or kind == "Mega" or kind == "Rainbow" or kind == "Gold" then
                        table.insert(out, {
                            obj = obj,
                            pos = pos,
                            kind = kind,
...
local function triggerPrompt(p)
    ...
    if fireproximityprompt then
        ...
        pcall(function()
            p:InputHoldBegin()
            task.wait(math.max(tonumber(p.HoldDuration) or 0, 0.05) + 0.05)
            p:InputHoldEnd()
        end)
    end
end
...
local function restartFarm()
    ...
    local ok, err = pcall(function()
        local url = BASE .. "core_v2.lua?cb=" .. tostring(math.floor(os.clock() * 1000))
        loadstring(game:HttpGet(url))()
    end)
    if not ok then warn("[GAG2 V3 EVENT] Failed to restart core:", err) end
end

local function collect(t)
    if busy or not t or not t.obj or not t.obj.Parent then return false end
    busy = true
    stopFarm()
    ...
    local success = (not obj.Parent) or obj.Parent ~= originalParent
    print("[GAG2 V3 EVENT]", success and "COLLECTED" or "ATTEMPTED", t.kind)

    if CFG.ReturnHome then
        local home = homePosition()
        if home then reach(home) end
    end
...
task.spawn(function()
    while running do
        if CFG.Enabled and not busy then
            for _, t in ipairs(candidates()) do
                local last = handled[t.obj]
                if t.obj.Parent and (not last or os.clock() - last >= CFG.RetryAfter) then
                    handled[t.obj] = os.clock()
                    pcall(function() collect(t) end)
                    break
                end
            end

            for obj, ts in pairs(handled) do
                if not obj.Parent or os.clock() - ts > 180 then handled[obj] = nil end
            end
        end
